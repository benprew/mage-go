...
func TestMathemagics(t *testing.T) {
	t.Run("target player draws 2^X cards where X=0", func(t *testing.T) {
		g := gametest.NewTestGame(t)
		g.AddCard(core.ZoneHand, gametest.PlayerA, "Mathematics")
		// X=0: target player draws 2^0 = 1 card
		g.CastSpellWithX(1, core.PrecombatMain, gametest.PlayerA, "Mathematics", 0, "PlayerA")
		g.StopAt(1, core.EndStep)
		g.Execute()
		// PlayerA cast the spell (hand was 1: Mathematics, then 0 after cast)
	...
	t.Run("target player draws 2^X cards where X=2", func(t *testing.T) {
		...
		g.CastSpellWithX(1, core.PrecombatMain, gametest.PlayerA, "Mathematics", 2, "PlayerB")
		g.StopAt(1, core.EndStep)
		g.Execute()
...
func TestMoltenNote(t *testing.T) {
	...
	t.Run("flashback — exiles after resolution", func(t *testing.T) {
		...
		giantPerm := tg.FindPermanentByName("Hill Giant", tg.GetPlayer(gametest.PlayerB).PlayerID())
		if err := tg.CastCardWithAlternateCost(pA.PlayerID(), cardID, 0, []uuid.UUID{giantPerm.ID()}, 0); err != nil {
...
func TestDuelTactics(t *testing.T) {
	...
	t.Run("flashback — exiles after resolution", func(t *testing.T) {
		...
		cardID := pA.Graveyard()[0].ID()
		bearPerm := tg.FindPermanentByName("Grizzly Bears", tg.GetPlayer(gametest.PlayerB).PlayerID())
		if err := tg.CastCardWithAlternateCost(pA.PlayerID(), cardID, 0, []uuid.UUID{bearPerm.ID()}, 0); err != nil {
			t.Fatalf("CastCardWithAlternateCost (flashback): %v", err)
		}
		tg.ResolveStack()
...
func TestLumaretsFavor(t *testing.T) {
	...
	t.Run("copies itself if controller gained life this turn", func(t *testing.T) {
		...
		g.FireEvent(core.GameEvent{Type: core.EvtLifeGained, PlayerID: p.PlayerID(), Amount: 1})
		g.CastSpell(1, core.PrecombatMain, gametest.PlayerA, "Lumaret's Favor", "Grizzly Bears")
...
func TestWiltInTheHeat(t *testing.T) {
	...
	t.Run("costs two less if a card left your graveyard this turn", func(t *testing.T) {
		...
		if got := g.ConditionalSpellCostReduction(pid, wilt); got != 0 {
		...
		if got := g.ConditionalSpellCostReduction(pid, wilt); got != 2 {
