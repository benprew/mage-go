...
func registerCreatures() {
	...
	Register("Jadzi, Steward of Fate // Oracle's Gift", func() Card {
		fractalFilter := NewPermanentFilter("Fractal", func(p *Permanent, _ *Game) bool {
			return slices.Contains(p.Card.SubTypes(), "Fractal")
		})
		spellFactory := func() Card {
		...
		return NewCreature("Grave Researcher // Reanimate", "{2}{B} // {B}", 3, 3,
			...
			WithAbility(BeginningOfUpkeepTrigger(
				FuncEffect(
					...
					EffectProperties{},
					func(g *Game, sourceID, controller uuid.UUID, _ []uuid.UUID) error {
						if err := ApplyEffect(g, Surveil(Fixed(1)), sourceID, controller, nil); err != nil {
							return err
						}
						p := g.GetPlayer(controller)
						if p == nil {
			...
			WithAbility(WheneverYouCastSpellTrigger(
				OpusEffect(
					...
					func(g *Game, sourceID, controller uuid.UUID, manaSpent int) error {
						...
						if manaSpent >= 5 {
							_ = ApplyEffect(g, GrantKeyword(DoubleStrike).Targeting(ToSource()).Until(EndOfTurn), sourceID, controller, nil)
						}
						return nil
					},
				),
		...
		spellFactory := func() Card {
			return NewSorcery("Awaken the Ages", "{5}{R}",
				NewSpellAbility(FuncEffect(
					...
					func(g *Game, sourceID, controller uuid.UUID, _ []uuid.UUID) error {
						for range 2 {
							eff := CreateColoredToken("Spirit Token", 2, 2,
								[]Color{Red, White},
								[]CardType{TypeCreature},
								[]string{"Spirit"},
							)
							if err := ApplyEffect(g, eff, sourceID, controller, nil); err != nil {
								return err
							}
						}
						return nil
