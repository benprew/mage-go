...
func registerSpells() {
			...
			NewSpellAbility(
				FuncEffect(
					...
					func(g *Game, sourceID, controller uuid.UUID, targets []uuid.UUID) error {
						...
						_ = ApplyEffect(g, eff, sourceID, controller, nil)
	...
	Register("Mathematics", func() Card {
		return NewSorcery("Mathematics", "{X}{X}{U}{U}",
	...
	Register("Run Behind", func() Card {
		return NewInstant("Run Behind", "{3}{U}",
			...
			WithTargetConditionalCostReduction(1, func(g *Game, controller uuid.UUID, _ Card, targets []uuid.UUID) bool {
				return slices.ContainsFunc(targets, g.IsAttackingInCombat)
			}),
		)
	})
