package limited

import (
	"testing"

	"github.com/mage/mage/pkg/mage"
)

// Ensure all Alpha card packages are imported.
var (
	_ = registerCreatures     //nolint:unused
	_ = registerSpells        //nolint:unused
	_ = registerEnchantments  //nolint:unused
	_ = registerArtifacts     //nolint:unused
	_ = registerLands         //nolint:unused
)

// =============================================================================
// Per-card correctness tests. Each function validates the correct MTG behavior
// for one card and fails when the implementation is wrong or incomplete.
// =============================================================================

func TestBlackLotus(t *testing.T) {
	t.Run("sacrificed on activation", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Black Lotus")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Black Lotus")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertPermanentCount(mage.PlayerA, "Black Lotus", 0)
	})

	t.Run("produces 3 mana of any one color", func(t *testing.T) {
		// Black Lotus should let the player choose a color. The current
		// implementation hardcodes Black via AddAnyMana(3, Black).
		// Auto-mana adds 5 of each color for ActivateAbility calls, so
		// the Lotus's 3 Black is detectable as pool.Count(Black) == 8.
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Black Lotus")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Black Lotus")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		pool := g.Players[0].ManaPool()
		blackCount := pool.Count(mage.Black)
		if blackCount > 5 {
			t.Errorf("Black Lotus is hardcoded to produce Black; got %d Black (5 auto + 3 Lotus). Should allow any color choice", blackCount)
		}
	})
}

func TestBerserk(t *testing.T) {
	t.Run("doubles target creature power", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Craw Wurm") // 6/4
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Berserk")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Berserk", "Craw Wurm")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertPowerToughness(mage.PlayerA, "Craw Wurm", 12, 4)
	})

	t.Run("destroys creature at end of turn", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Grizzly Bears")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Berserk")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Berserk", "Grizzly Bears")
		g.StopAt(2, mage.Upkeep)
		g.Execute()
		g.AssertPermanentCount(mage.PlayerA, "Grizzly Bears", 0)
	})
}

func TestWheelOfFortune(t *testing.T) {
	t.Run("discards hand then draws 7", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Wheel of Fortune")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Grizzly Bears")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Hill Giant")
		for i := 0; i < 10; i++ {
			g.AddCard(mage.ZoneLibrary, mage.PlayerA, "Grizzly Bears")
		}
		for i := 0; i < 10; i++ {
			g.AddCard(mage.ZoneLibrary, mage.PlayerB, "Grizzly Bears")
		}
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Wheel of Fortune")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		playerA := g.Players[0]
		if len(playerA.Hand()) != 7 {
			t.Errorf("expected 7 cards in hand after Wheel, got %d", len(playerA.Hand()))
		}
		// Bears and Giant should have been discarded before draw-7
		g.AssertGraveyardCount(mage.PlayerA, "Grizzly Bears", 1)
		g.AssertGraveyardCount(mage.PlayerA, "Hill Giant", 1)
	})
}

func TestTimetwister(t *testing.T) {
	t.Run("shuffles graveyard into library", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Timetwister")
		g.AddCard(mage.ZoneGraveyard, mage.PlayerA, "Lightning Bolt")
		g.AddCard(mage.ZoneGraveyard, mage.PlayerA, "Grizzly Bears")
		for i := 0; i < 10; i++ {
			g.AddCard(mage.ZoneLibrary, mage.PlayerA, "Forest")
		}
		for i := 0; i < 7; i++ {
			g.AddCard(mage.ZoneLibrary, mage.PlayerB, "Forest")
		}
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Timetwister")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertGraveyardCount(mage.PlayerA, "Lightning Bolt", 0)
		g.AssertGraveyardCount(mage.PlayerA, "Grizzly Bears", 0)
	})
}

func TestHowlFromBeyond(t *testing.T) {
	t.Run("gives +X/+0", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Grizzly Bears") // 2/2
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Howl from Beyond")
		g.CastSpellWithX(1, mage.PrecombatMain, mage.PlayerA, "Howl from Beyond", 5, "Grizzly Bears")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertPowerToughness(mage.PlayerA, "Grizzly Bears", 7, 2)
	})
}

func TestGuardianAngel(t *testing.T) {
	t.Run("prevents X damage to creature", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Grizzly Bears") // 2/2
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Lightning Bolt")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Guardian Angel")
		// Prevent 3 damage to Bears, then Bolt them
		g.CastSpellWithX(1, mage.PrecombatMain, mage.PlayerA, "Guardian Angel", 3, "Grizzly Bears")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Lightning Bolt", "Grizzly Bears")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertPermanentCount(mage.PlayerA, "Grizzly Bears", 1)
	})
}

func TestFirebreathingAura(t *testing.T) {
	t.Run("grants repeatable +1/+0 activated ability", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Grizzly Bears") // 2/2
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Firebreathing")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Firebreathing", "Grizzly Bears")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Grizzly Bears")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Grizzly Bears")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		// {R}: +1/+0, activated twice = 2+2/2 = 4/2
		g.AssertPowerToughness(mage.PlayerA, "Grizzly Bears", 4, 2)
	})
}

func TestBlessingAura(t *testing.T) {
	t.Run("grants repeatable +1/+1 activated ability", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Grizzly Bears") // 2/2
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Blessing")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Blessing", "Grizzly Bears")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Grizzly Bears")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Grizzly Bears")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		// {W}: +1/+1, activated twice = 2+2/2+2 = 4/4
		g.AssertPowerToughness(mage.PlayerA, "Grizzly Bears", 4, 4)
	})
}

func TestEarthbind(t *testing.T) {
	t.Run("removes flying from enchanted creature", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerB, "Air Elemental") // 4/4 Flying
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Earthbind")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Earthbind", "Air Elemental")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertHasAbility(mage.PlayerB, "Air Elemental", mage.Flying, false)
	})
}

func TestParalyze(t *testing.T) {
	t.Run("creature stays tapped through untap step", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerB, "Hill Giant")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Paralyze")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Paralyze", "Hill Giant")
		// Turn 2 is PlayerB's turn; creature should remain tapped
		g.StopAt(2, mage.PrecombatMain)
		g.Execute()
		g.AssertTapped(mage.PlayerB, "Hill Giant", true)
	})
}

func TestForceOfNature(t *testing.T) {
	t.Run("deals 8 damage on upkeep if not paid", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Force of Nature")
		g.StopAt(1, mage.PrecombatMain)
		g.Execute()
		// Upkeep cost is {G}{G}{G}{G} or take 8 damage
		g.AssertLife(mage.PlayerA, 12)
	})
}

func TestLordOfThePit(t *testing.T) {
	t.Run("sacrifices creature on upkeep instead of dealing damage", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Lord of the Pit")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Scathe Zombies") // sacrifice fodder
		g.StopAt(1, mage.PrecombatMain)
		g.Execute()
		g.AssertPermanentCount(mage.PlayerA, "Scathe Zombies", 0)
		g.AssertLife(mage.PlayerA, 20) // no damage when creature is sacrificed
	})
}

func TestPhantasmalForces(t *testing.T) {
	t.Run("sacrificed if upkeep cost not paid", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Phantasmal Forces")
		g.StopAt(1, mage.PrecombatMain)
		g.Execute()
		// Upkeep cost is {U} or sacrifice
		g.AssertPermanentCount(mage.PlayerA, "Phantasmal Forces", 0)
	})
}

func TestSengirVampire(t *testing.T) {
	t.Run("no counter when another source kills creature", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Sengir Vampire") // 4/4 Flying
		g.AddCard(mage.ZoneBattlefield, mage.PlayerB, "Grizzly Bears")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Lightning Bolt")
		// Kill Bears with Bolt, not Sengir's combat damage
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Lightning Bolt", "Grizzly Bears")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertCounterCount(mage.PlayerA, "Sengir Vampire", mage.P1P1, 0)
	})
}

func TestDrudgeSkeletons(t *testing.T) {
	t.Run("regenerates from lethal damage", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Drudge Skeletons") // 1/1
		g.AddCard(mage.ZoneHand, mage.PlayerB, "Lightning Bolt")
		// Activate regen on Upkeep, Bolt on PrecombatMain
		g.ActivateAbility(1, mage.Upkeep, mage.PlayerA, "Drudge Skeletons")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerB, "Lightning Bolt", "Drudge Skeletons")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertPermanentCount(mage.PlayerA, "Drudge Skeletons", 1)
	})
}

func TestSamiteHealer(t *testing.T) {
	t.Run("prevents 1 damage to creature", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Samite Healer")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Merfolk of the Pearl Trident") // 1/1
		g.AddCard(mage.ZoneBattlefield, mage.PlayerB, "Prodigal Sorcerer")
		// Healer prevents 1, Tim deals 1 — Merfolk should survive
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Samite Healer", "Merfolk of the Pearl Trident")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerB, "Prodigal Sorcerer", "Merfolk of the Pearl Trident")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertPermanentCount(mage.PlayerA, "Merfolk of the Pearl Trident", 1)
	})
}

func TestCopperTablet(t *testing.T) {
	t.Run("damages each player on their upkeep", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Copper Tablet")
		g.StopAt(2, mage.PrecombatMain)
		g.Execute()
		g.AssertLife(mage.PlayerA, 19) // turn 1 upkeep
		g.AssertLife(mage.PlayerB, 19) // turn 2 upkeep
	})
}

func TestKarma(t *testing.T) {
	t.Run("deals damage per Swamp controlled", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Karma")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerB, "Swamp", 3)
		g.StopAt(2, mage.PrecombatMain)
		g.Execute()
		g.AssertLife(mage.PlayerB, 17) // 3 Swamps = 3 damage
	})
}

func TestBlackVise(t *testing.T) {
	t.Run("deals hand-size damage to opponent", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Black Vise")
		for i := 0; i < 7; i++ {
			g.AddCard(mage.ZoneHand, mage.PlayerB, "Forest")
		}
		g.StopAt(2, mage.PrecombatMain)
		g.Execute()
		// 7 cards - 4 threshold = 3 damage to PlayerB
		g.AssertLife(mage.PlayerB, 17)
		g.AssertLife(mage.PlayerA, 20) // controller unaffected
	})
}

func TestAnimateDead(t *testing.T) {
	t.Run("returns creature from graveyard to battlefield", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneGraveyard, mage.PlayerA, "Serra Angel") // 4/4
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Animate Dead")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Animate Dead", "Serra Angel")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertPermanentCount(mage.PlayerA, "Serra Angel", 1)
		g.AssertPowerToughness(mage.PlayerA, "Serra Angel", 3, 4) // -1/-0 from Animate Dead
	})
}

func TestVerduranEnchantress(t *testing.T) {
	t.Run("draws on any enchantment cast not just green", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Verduran Enchantress")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Grizzly Bears")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Holy Armor") // white enchantment
		g.AddCard(mage.ZoneLibrary, mage.PlayerA, "Grizzly Bears")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Holy Armor", "Grizzly Bears")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		playerA := g.Players[0]
		if len(playerA.Hand()) < 1 {
			t.Errorf("Verduran Enchantress should draw on any enchantment cast, hand has %d cards", len(playerA.Hand()))
		}
	})
}

func TestBlueElementalBlast(t *testing.T) {
	t.Run("cannot counter green spell", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Grizzly Bears")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Giant Growth")
		g.AddCard(mage.ZoneHand, mage.PlayerB, "Blue Elemental Blast")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Giant Growth", "Grizzly Bears")
		g.CastInResponseTo(mage.PlayerB, "Blue Elemental Blast")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		// BEB only counters red; Giant Growth is green so it should resolve
		g.AssertPowerToughness(mage.PlayerA, "Grizzly Bears", 5, 5)
	})
}

func TestRedElementalBlast(t *testing.T) {
	t.Run("cannot counter green spell", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Grizzly Bears")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Giant Growth")
		g.AddCard(mage.ZoneHand, mage.PlayerB, "Red Elemental Blast")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Giant Growth", "Grizzly Bears")
		g.CastInResponseTo(mage.PlayerB, "Red Elemental Blast")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		// REB only counters blue; Giant Growth is green so it should resolve
		g.AssertPowerToughness(mage.PlayerA, "Grizzly Bears", 5, 5)
	})
}

func TestDeathgrip(t *testing.T) {
	t.Run("cannot counter red spell", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerB, "Deathgrip")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Lightning Bolt")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Lightning Bolt", "PlayerB")
		g.ActivateInResponseTo(mage.PlayerB, "Deathgrip")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		// Deathgrip only counters green; Bolt is red so it should resolve
		g.AssertLife(mage.PlayerB, 17)
	})
}

func TestLifeforce(t *testing.T) {
	t.Run("cannot counter red spell", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerB, "Lifeforce")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Lightning Bolt")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Lightning Bolt", "PlayerB")
		g.ActivateInResponseTo(mage.PlayerB, "Lifeforce")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		// Lifeforce only counters black; Bolt is red so it should resolve
		g.AssertLife(mage.PlayerB, 17)
	})
}

func TestSpellBlast(t *testing.T) {
	t.Run("fails when X is less than target CMC", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Grizzly Bears")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Giant Growth") // CMC 1
		g.AddCard(mage.ZoneHand, mage.PlayerB, "Spell Blast")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Giant Growth", "Grizzly Bears")
		g.CastInResponseToWithX(mage.PlayerB, "Spell Blast", 0) // X=0 < CMC 1
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertPowerToughness(mage.PlayerA, "Grizzly Bears", 5, 5)
	})
}

func TestPowerSink(t *testing.T) {
	t.Run("does not counter if opponent can pay X", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Grizzly Bears")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Giant Growth")
		g.AddCard(mage.ZoneHand, mage.PlayerB, "Power Sink")
		g.Players[0].ManaPool().Add(mage.Colorless, 5) // extra mana to pay
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Giant Growth", "Grizzly Bears")
		g.CastInResponseToWithX(mage.PlayerB, "Power Sink", 1)
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		// PlayerA has mana to pay X=1, so spell should resolve
		g.AssertPowerToughness(mage.PlayerA, "Grizzly Bears", 5, 5)
	})
}

func TestTwiddle(t *testing.T) {
	t.Run("can untap a tapped permanent", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Icy Manipulator")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Hill Giant")
		g.ActivateAbility(1, mage.Upkeep, mage.PlayerA, "Icy Manipulator", "Hill Giant")
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Twiddle")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Twiddle", "Hill Giant")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertTapped(mage.PlayerA, "Hill Giant", false)
	})
}

func TestBasaltMonolith(t *testing.T) {
	t.Run("does not untap during untap step", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Basalt Monolith")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Basalt Monolith")
		g.StopAt(3, mage.PrecombatMain) // PlayerA's next untap step
		g.Execute()
		g.AssertTapped(mage.PlayerA, "Basalt Monolith", true)
	})
}

func TestManaVault(t *testing.T) {
	t.Run("does not untap during untap step", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Mana Vault")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Mana Vault")
		g.StopAt(3, mage.PrecombatMain)
		g.Execute()
		g.AssertTapped(mage.PlayerA, "Mana Vault", true)
	})
}

func TestGauntletOfMight(t *testing.T) {
	t.Run("Mountains produce extra red mana", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Gauntlet of Might")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Mountain")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Mountain")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		// Auto-mana adds 5R. Mountain tapped for 1R + Gauntlet bonus 1R = 7R total.
		pool := g.Players[0].ManaPool()
		if pool.Count(mage.Red) < 7 {
			t.Errorf("Gauntlet should make Mountain produce extra {R}; expected >=7 red mana, got %d", pool.Count(mage.Red))
		}
	})
}

func TestNevinyrralsDisk(t *testing.T) {
	t.Run("enters the battlefield tapped", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Nevinyrral's Disk")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Nevinyrral's Disk")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertTapped(mage.PlayerA, "Nevinyrral's Disk", true)
	})
}

func TestDragonWhelp(t *testing.T) {
	t.Run("destroyed at EOT if pumped 4 or more times", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Dragon Whelp") // 2/3 Flying
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Dragon Whelp")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Dragon Whelp")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Dragon Whelp")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Dragon Whelp")
		g.StopAt(2, mage.Upkeep)
		g.Execute()
		g.AssertPermanentCount(mage.PlayerA, "Dragon Whelp", 0)
	})
}

func TestZombieMaster(t *testing.T) {
	t.Run("grants regeneration to other Zombies", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Zombie Master")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Scathe Zombies") // 2/2 Zombie
		g.AddCard(mage.ZoneHand, mage.PlayerB, "Lightning Bolt")
		g.ActivateAbility(1, mage.Upkeep, mage.PlayerA, "Scathe Zombies")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerB, "Lightning Bolt", "Scathe Zombies")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertPermanentCount(mage.PlayerA, "Scathe Zombies", 1)
	})
}

func TestGoblinBalloonBrigade(t *testing.T) {
	t.Run("gains flying when activated", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Goblin Balloon Brigade")
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Goblin Balloon Brigade")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertHasAbility(mage.PlayerA, "Goblin Balloon Brigade", mage.Flying, true)
	})
}

func TestDwarvenWarriors(t *testing.T) {
	t.Run("makes power-2-or-less creature unblockable", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Dwarven Warriors")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Grizzly Bears")  // 2/2
		g.AddCard(mage.ZoneBattlefield, mage.PlayerB, "Hill Giant")     // blocker
		g.ActivateAbility(1, mage.PrecombatMain, mage.PlayerA, "Dwarven Warriors", "Grizzly Bears")
		g.Attack(1, mage.PlayerA, "Grizzly Bears")
		g.Block(1, mage.PlayerB, "Hill Giant", "Grizzly Bears")
		g.StopAt(1, mage.EndCombat)
		g.Execute()
		g.AssertLife(mage.PlayerB, 18)
	})
}

func TestSedgeTroll(t *testing.T) {
	t.Run("gets +1/+1 while you control a Swamp", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Sedge Troll") // base 2/2
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Swamp")
		g.StopAt(1, mage.PrecombatMain)
		g.Execute()
		g.AssertPowerToughness(mage.PlayerA, "Sedge Troll", 3, 3)
	})
}

func TestTwoHeadedGiant(t *testing.T) {
	t.Run("can block two attackers", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerB, "Two-Headed Giant of Foriys") // 4/4
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Grizzly Bears")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Gray Ogre")
		g.Attack(1, mage.PlayerA, "Grizzly Bears", "Gray Ogre")
		g.Block(1, mage.PlayerB, "Two-Headed Giant of Foriys", "Grizzly Bears")
		g.Block(1, mage.PlayerB, "Two-Headed Giant of Foriys", "Gray Ogre")
		g.StopAt(1, mage.EndCombat)
		g.Execute()
		g.AssertLife(mage.PlayerB, 20) // blocks both, no damage through
	})
}

func TestJuggernaut(t *testing.T) {
	t.Run("cannot be blocked by Walls", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Juggernaut")    // 5/3
		g.AddCard(mage.ZoneBattlefield, mage.PlayerB, "Wall of Stone") // 0/8 Defender
		g.Attack(1, mage.PlayerA, "Juggernaut")
		g.Block(1, mage.PlayerB, "Wall of Stone", "Juggernaut")
		g.StopAt(1, mage.EndCombat)
		g.Execute()
		g.AssertLife(mage.PlayerB, 15)
	})
}

func TestNettlingImp(t *testing.T) {
	t.Run("has activated ability to force attack", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Nettling Imp")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerB, "Hill Giant")
		g.ActivateAbility(2, mage.PrecombatMain, mage.PlayerA, "Nettling Imp", "Hill Giant")
		g.StopAt(2, mage.BeginCombat)
		g.Execute()
		g.AssertTapped(mage.PlayerA, "Nettling Imp", true)
	})
}

func TestSeaSerpent(t *testing.T) {
	t.Run("sacrificed without Islands", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Sea Serpent")
		g.StopAt(1, mage.PrecombatMain)
		g.Execute()
		g.AssertPermanentCount(mage.PlayerA, "Sea Serpent", 0)
	})
}

func TestNetherShadow(t *testing.T) {
	t.Run("returns from graveyard with 3 creatures above", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneGraveyard, mage.PlayerA, "Nether Shadow")
		g.AddCard(mage.ZoneGraveyard, mage.PlayerA, "Grizzly Bears")
		g.AddCard(mage.ZoneGraveyard, mage.PlayerA, "Hill Giant")
		g.AddCard(mage.ZoneGraveyard, mage.PlayerA, "Gray Ogre")
		g.StopAt(1, mage.PrecombatMain)
		g.Execute()
		g.AssertPermanentCount(mage.PlayerA, "Nether Shadow", 1)
	})
}

func TestRegeneration(t *testing.T) {
	// Creatures that should have a regeneration activated ability but don't.
	tests := []struct {
		name      string
		toughness int
	}{
		{"Will-o'-the-Wisp", 1},
		{"Uthden Troll", 2},
		{"Wall of Bone", 4},
		{"Wall of Brambles", 3},
		{"Living Wall", 6},
	}
	for _, tt := range tests {
		t.Run(tt.name+" survives lethal via regen", func(t *testing.T) {
			g := mage.NewTestGame(t)
			g.AddCard(mage.ZoneBattlefield, mage.PlayerA, tt.name)
			g.AddCard(mage.ZoneHand, mage.PlayerB, "Fireball")
			g.ActivateAbility(1, mage.Upkeep, mage.PlayerA, tt.name)
			g.CastSpellWithX(1, mage.PrecombatMain, mage.PlayerB, "Fireball", tt.toughness+1, tt.name)
			g.StopAt(1, mage.BeginCombat)
			g.Execute()
			g.AssertPermanentCount(mage.PlayerA, tt.name, 1)
		})
	}
}

func TestAspectOfWolf(t *testing.T) {
	t.Run("is an Enchantment not a Creature", func(t *testing.T) {
		card, err := mage.CreateCard("Aspect of Wolf")
		if err != nil {
			t.Fatalf("could not create Aspect of Wolf: %v", err)
		}
		if card.HasType(mage.TypeCreature) {
			t.Errorf("Aspect of Wolf should NOT be a Creature, it is an Enchantment - Aura")
		}
		if !card.HasType(mage.TypeEnchantment) {
			t.Errorf("Aspect of Wolf should be an Enchantment, got types: %v", card.Types())
		}
	})
}

func TestNightmare(t *testing.T) {
	t.Run("P/T equals Swamp count", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Nightmare")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Swamp", 3)
		g.StopAt(1, mage.PrecombatMain)
		g.Execute()
		g.AssertPermanentCount(mage.PlayerA, "Nightmare", 1)
		g.AssertPowerToughness(mage.PlayerA, "Nightmare", 3, 3)
	})
}

func TestPlagueRats(t *testing.T) {
	t.Run("P/T scales with Plague Rats count", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Plague Rats", 3)
		g.StopAt(1, mage.PrecombatMain)
		g.Execute()
		found := false
		for _, perm := range g.Battlefield {
			if perm.Name() == "Plague Rats" {
				p := perm.CurrentPower(g.Game)
				tt := perm.CurrentToughness(g.Game)
				if p == 3 && tt == 3 {
					found = true
				} else {
					t.Errorf("each Plague Rats should be 3/3 with 3 rats, got %d/%d", p, tt)
				}
			}
		}
		if !found {
			t.Errorf("no Plague Rats found as 3/3")
		}
	})
}

func TestClone(t *testing.T) {
	t.Run("copies target creature on ETB", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Serra Angel") // 4/4
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Clone")
		g.CastSpell(1, mage.PrecombatMain, mage.PlayerA, "Clone", "Serra Angel")
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		// Should have two 4/4 creatures (original + clone)
		count44 := 0
		for _, perm := range g.Battlefield {
			if perm.Controller == g.Players[0].PlayerID() {
				if perm.CurrentPower(g.Game) == 4 && perm.CurrentToughness(g.Game) == 4 {
					count44++
				}
			}
		}
		if count44 < 2 {
			t.Errorf("Clone should create a second 4/4 (copy of Serra Angel); found %d", count44)
		}
		g.AssertGraveyardCount(mage.PlayerA, "Clone", 0)
	})
}

func TestKeldonWarlord(t *testing.T) {
	t.Run("P/T equals non-Wall creature count", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Keldon Warlord")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Grizzly Bears")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Hill Giant")
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Wall of Stone") // Wall, shouldn't count
		g.StopAt(1, mage.PrecombatMain)
		g.Execute()
		// 3 non-Wall creatures: Warlord + Bears + Giant
		g.AssertPermanentCount(mage.PlayerA, "Keldon Warlord", 1)
		g.AssertPowerToughness(mage.PlayerA, "Keldon Warlord", 3, 3)
	})
}

func TestRockHydra(t *testing.T) {
	t.Run("enters with X +1/+1 counters", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneHand, mage.PlayerA, "Rock Hydra")
		g.CastSpellWithX(1, mage.PrecombatMain, mage.PlayerA, "Rock Hydra", 4)
		g.StopAt(1, mage.BeginCombat)
		g.Execute()
		g.AssertPermanentCount(mage.PlayerA, "Rock Hydra", 1)
		g.AssertPowerToughness(mage.PlayerA, "Rock Hydra", 4, 4)
	})
}

func TestClockworkBeast(t *testing.T) {
	t.Run("loses a counter when attacking", func(t *testing.T) {
		g := mage.NewTestGame(t)
		g.AddCard(mage.ZoneBattlefield, mage.PlayerA, "Clockwork Beast")
		g.Attack(1, mage.PlayerA, "Clockwork Beast")
		g.StopAt(1, mage.EndCombat)
		g.Execute()
		// 7 +1/+0 counters on 0/4 base = 7/4; after attack loses 1 = 6/4
		g.AssertPowerToughness(mage.PlayerA, "Clockwork Beast", 6, 4)
	})
}
