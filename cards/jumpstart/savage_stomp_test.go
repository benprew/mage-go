...
func permIDByName(g *gametest.TestGame, p gametest.PlayerRef, name string) uuid.UUID {
	...
	if perm := g.Game.FindPermanentByName(name, pid); perm != nil {
		return perm.ID()
	}
	return uuid.Nil
}
...
func TestSavageStomp_DinosaurTargetReducesCostByTwo(t *testing.T) {
	...
	if got := g.Game.ConditionalSpellCostReduction(pid, stomp); got != 0 {
		t.Errorf("pre-target reduction: got %d, want 0", got)
	}

	// With targets context: targeting our Dinosaur first triggers the {2}
	// reduction.
	got := g.Game.ConditionalSpellCostReductionWithTargets(pid, stomp, []uuid.UUID{dino, bears})
	if got != 2 {
...
func TestSavageStomp_NonDinosaurTargetNoReduction(t *testing.T) {
	...
	got := g.Game.ConditionalSpellCostReductionWithTargets(pid, stomp, []uuid.UUID{myBears, giant})
	if got != 0 {
...
func TestSavageStomp_OpponentDinosaurDoesNotReduce(t *testing.T) {
	...
	got := g.Game.ConditionalSpellCostReductionWithTargets(pid, stomp, []uuid.UUID{myBears, oppDino})
	if got != 0 {
