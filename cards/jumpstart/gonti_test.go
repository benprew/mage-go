...
func findGontiExiledCard(t *testing.T, g *gametest.TestGame, playerID uuid.UUID) (*mage.ExiledCard, uuid.UUID) {
	t.Helper()
	exile := g.Game.GetExile()
	for i := range exile {
		ec := &exile[i]
		if g.Game.CastFromExilePermissionFor(playerID, ec.Card.ID()) != nil {
...
func TestGontiLordOfLuxury_ControllerCastsExiledCardWithMatchingMana(t *testing.T) {
	...
	if err := g.Game.CastExiledCardWithPermission(pA.PlayerID(), cardID, []uuid.UUID{pB.PlayerID()}, 0); err != nil {
	...
	if g.Game.FindExiledCard(cardID) != nil {
		t.Errorf("expected card removed from exile after cast")
	}
	if g.Game.CastFromExilePermissionFor(pA.PlayerID(), cardID) != nil {
...
func TestGontiLordOfLuxury_OpponentCannotCastExiledCard(t *testing.T) {
	...
	if err := g.Game.CastExiledCardWithPermission(pB.PlayerID(), cardID, []uuid.UUID{pA.PlayerID()}, 0); err == nil {
