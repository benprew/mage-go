...
func ExecuteEffect(ctx *EffectContext, data Effect) error {
	...
	case *returnSourceFromGraveyardToBattlefieldEffect:
		return execReturnSourceFromGraveyardToBattlefield(ctx, e)
...
func execLoseLifeDynamic(ctx *EffectContext, e *loseLifeDynamicEffect) error {
	amount := e.amount.Resolve(ctx.Game, ctx.SourceID, ctx.Controller, ctx.Targets)
	if amount <= 0 {
		return nil
	}
