...
// buildState is a helper that returns a minimal GameState with the given player IDs.
func buildState(youID, oppID uuid.UUID, youName, oppName string) *interactive.GameState {
...
func anyTargetOpt() interactive.ActionOption {
	return interactive.ActionOption{
		Type:        interactive.ActionCastSpell,
		NeedsTarget: true,
	}
}

func TestGetTargetChoices_IncludesBothPlayers(t *testing.T) {
	youID := uuid.New()
	oppID := uuid.New()
	state := buildState(youID, oppID, "Alice", "Bob")

	ids, labels := interactive.GetTargetChoices(state, anyTargetOpt())

	if !containsID(ids, youID) {
		t.Error("GetTargetChoices should include the human player's UUID")
	}
	if !containsID(ids, oppID) {
		t.Error("GetTargetChoices should include the opponent player's UUID")
	}
	_ = labels
}

func TestGetTargetChoices_PlayerLabels(t *testing.T) {
	youID := uuid.New()
	oppID := uuid.New()
	state := buildState(youID, oppID, "Alice", "Bob")
	...
	oppIdx := indexOfID(ids, oppID)
	if oppIdx < 0 {
		t.Fatal("opponent ID not found in choices")
	}
	if labels[oppIdx] != "Bob (player)" {
		t.Errorf("opponent label = %q, want %q", labels[oppIdx], "Bob (player)")
	}

	youIdx := indexOfID(ids, youID)
	if youIdx < 0 {
		t.Fatal("you ID not found in choices")
	}
	if labels[youIdx] != "You (player)" {
		t.Errorf("self label = %q, want %q", labels[youIdx], "You (player)")
	}
}

func TestGetTargetChoices_NilPlayerIDExcluded(t *testing.T) {
	// If a player ID is uuid.Nil (missing from snapshot), it must not appear.
	state := buildState(uuid.Nil, uuid.Nil, "Alice", "Bob")

	ids, _ := interactive.GetTargetChoices(state, anyTargetOpt())

	if containsID(ids, uuid.Nil) {
		t.Error("GetTargetChoices must not include uuid.Nil as a target")
	}
}

func TestGetTargetChoices_IncludesOpponentCreatures(t *testing.T) {
	youID := uuid.New()
	oppID := uuid.New()
	creatureID := uuid.New()

	state := buildState(youID, oppID, "Alice", "Bob")
	state.Opponent.Battlefield = []interactive.PermanentState{
		{ID: creatureID, Name: "Grizzly Bears", Power: 2, Toughness: 2, IsCreature: true},
	}

	ids, labels := interactive.GetTargetChoices(state, anyTargetOpt())

	if !containsID(ids, creatureID) {
		t.Error("GetTargetChoices should include opponent creatures")
	}
	idx := indexOfID(ids, creatureID)
	want := "Grizzly Bears 2/2 (Bob)"
	if labels[idx] != want {
		t.Errorf("creature label = %q, want %q", labels[idx], want)
	}
}

func TestGetTargetChoices_IncludesYourCreatures(t *testing.T) {
	youID := uuid.New()
	oppID := uuid.New()
	creatureID := uuid.New()

	state := buildState(youID, oppID, "Alice", "Bob")
	state.You.Battlefield = []interactive.PermanentState{
		{ID: creatureID, Name: "Serra Angel", Power: 4, Toughness: 4, IsCreature: true},
	}

	ids, labels := interactive.GetTargetChoices(state, anyTargetOpt())

	if !containsID(ids, creatureID) {
		t.Error("GetTargetChoices should include your own creatures")
	}
	idx := indexOfID(ids, creatureID)
	want := "Serra Angel 4/4 (You)"
	if labels[idx] != want {
		t.Errorf("creature label = %q, want %q", labels[idx], want)
	}
}

func TestGetTargetChoices_ExcludesNonCreatures(t *testing.T) {
	youID := uuid.New()
	oppID := uuid.New()
	landID := uuid.New()
	...
	state := buildState(youID, oppID, "Alice", "Bob")
	state.Opponent.Battlefield = []interactive.PermanentState{
		{ID: landID, Name: "Plains", IsLand: true, IsCreature: false},
	}

	ids, _ := interactive.GetTargetChoices(state, anyTargetOpt())

	if containsID(ids, landID) {
		t.Error("GetTargetChoices must not include non-creature permanents")
	}
}

func TestGetTargetChoices_LengthMatchesLabels(t *testing.T) {
	youID := uuid.New()
	oppID := uuid.New()
	c1 := uuid.New()
	c2 := uuid.New()
	...
	state := buildState(youID, oppID, "Alice", "Bob")
	state.Opponent.Battlefield = []interactive.PermanentState{
		{ID: c1, Name: "Ogre", Power: 3, Toughness: 3, IsCreature: true},
	}
	state.You.Battlefield = []interactive.PermanentState{
		{ID: c2, Name: "Elf", Power: 1, Toughness: 1, IsCreature: true},
	}

	ids, labels := interactive.GetTargetChoices(state, anyTargetOpt())

	if len(ids) != len(labels) {
		t.Errorf("len(ids)=%d != len(labels)=%d", len(ids), len(labels))
	}
	// 1 opp creature + 1 your creature + 2 players = 4
	if len(ids) != 4 {
		t.Errorf("expected 4 targets, got %d: %v", len(ids), labels)
	}
}

func TestGetTargetChoices_EmptyBattlefield_StillHasPlayers(t *testing.T) {
	youID := uuid.New()
	oppID := uuid.New()
	state := buildState(youID, oppID, "Alice", "Bob")
	...
	if len(ids) != 2 {
		t.Errorf("expected 2 targets (both players), got %d", len(ids))
	}
}

// ── SnapshotGameState + GetTargetChoices integration ────────────────────────

func TestGetTargetChoices_WithRealSnapshot(t *testing.T) {
	g, pa, pb := newTestGame()
	snap := interactive.SnapshotGameState(g, 0)

	ids, _ := interactive.GetTargetChoices(snap, anyTargetOpt())

	if !containsID(ids, pa.PlayerID()) {
		t.Error("target choices from real snapshot should include player A UUID")
	}
	if !containsID(ids, pb.PlayerID()) {
		t.Error("target choices from real snapshot should include player B UUID")
	}
	for _, id := range ids {
		if id == uuid.Nil {
			t.Error("target choices from real snapshot must not contain uuid.Nil")
		}
	}
}
